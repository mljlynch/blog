---
title: The Pernicious "The OS Watchdog Terminated Your App"
date: 2025-08-10
tags:
  - React Native
---
...
## 2. Loading too much data and rendering too many items in FlatLists.

We have a number of tricky FlatLists where we render a lot of SVGs as logos. These SVGs are relatively large (100+ SVGs x x mb per SVG). When we fetch the endpoint they are stored in App memory due to our use of react-query.

## 3. Images not being optimized for production.
...
## 4. Changing state on scroll

This seems like a dumb mistake in retrospect, but we had a screen on our app where we were setting the scroll height in state. This lead to the screen rerendering on scroll which lowered JS framerate to a crawl. The solution was to only update the state when the scroll height passed certain thresholds - in our case, there was one scroll height threshold we wanted to track.
